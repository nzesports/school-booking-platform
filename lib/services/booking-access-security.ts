...
export function newVerificationCode() { return String(randomInt(0, 100_000_000)).padStart(8, "0"); }
export function verificationCodeHash(challenge: string, code: string, secret: string) {
  return createHmac("sha256", secret).update(`booking-code:v1:${challenge}:${code}`).digest("hex");
}
