-- Reference-only browser access. The server validates a signed, booking-scoped
-- cookie before calling this service-role-only mutation. Keep the original
-- migration intact for environments that already applied the email-link flow.
create function public.change_reference_booking_session(
  p_reference text, p_booking_id uuid, p_session_id uuid, p_action text,
...
begin
  ...
  select s.* into session from booking_sessions s
    ...
    where s.id = p_session_id and b.id = p_booking_id
      and b.reference_code = p_reference for update of s;
  ...
  insert into booking_activity_logs(booking_request_id, booking_session_id, action, actor_type, details)
    values(p_booking_id, session.id, 'session.' || case when p_action = 'cancel' then 'cancelled' else 'rescheduled' end || '_by_school', 'school',
      jsonb_build_object('access', 'booking_reference', 'previous_starts_at', session.starts_at,
        'starts_at', coalesce(p_new_start, session.starts_at), 'notes', left(p_notes, 2000)));
  return true;
end;
$$;
revoke all on function public.change_reference_booking_session(text, uuid, uuid, text, timestamptz, text, timestamptz, timestamptz, text) from public, anon, authenticated;
grant execute on function public.change_reference_booking_session(text, uuid, uuid, text, timestamptz, text, timestamptz, timestamptz, text) to service_role;

-- The previous email-grant mutation is no longer used.
revoke execute on function public.change_guest_booking_session(text, uuid, uuid, text, timestamptz, text, timestamptz, timestamptz, text) from service_role;
